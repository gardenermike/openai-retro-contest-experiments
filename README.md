...
#### PPO2
[PPO2](https://blog.openai.com/openai-baselines-ppo/) is a high-performing policy gradient algorithm. Instead of optimizing the _reward_ per-action, as DQN algorithms do, policy gradient algorithms optimize an explicit _policy_ to take per action, improving it iteratively during training. PPO (and the extension, PPO2) uses a clever clipping approach to prevent excessive updates to the policy without needing expensive calculation, allowing for stable and efficient training.
